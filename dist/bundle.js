...
function Codeck(deck, charset) {
  ...
  this.maxLength = floor$2(log$2(factorial) / log$2(this.charset.length));
}
...
var urlsToCache = ['/', '/static/style.css', 'https://unpkg.com/big.js@3.1.3/big.min.js', 'https://unpkg.com/d3@4.7.1/build/d3.min.js', '/dist/bundle.js'];
...
var svg = select('#cards svg').attr('width', '600px').attr('height', '400px');

var messageInput = select('#message');
var cleanedMessageSpan = select('.cleanednotification');
...
function cleanMessage(text$$1) {
  return text$$1.toUpperCase().trim().replace(/['"]/g, ',').replace(/[!?]/g, '.').replace(/[^A-Z0-9,. ]/g, '');
}
